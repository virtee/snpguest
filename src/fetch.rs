...
mod crl {
    ...
    pub fn fetch_crl(args: Args) -> Result<()> {
        ...
        let bytes: Vec<u8> = match args.encoding {
            CertFormat::Pem => match encode_string("X509 CRL", LineEnding::LF, &crl_der) {
                Ok(crl_pem) => crl_pem.into(),
                _ => return Err(anyhow::anyhow!("Could not convert CRL from DER to PEM")),
            },
            CertFormat::Der => crl_der,
        };
        ...
        let mut file = if crl_path.exists() {
            std::fs::OpenOptions::new()
                .write(true)
                .truncate(true)
                .open(crl_path)
                .context("Unable to overwrite CRL contents")?
        } else {
            fs::File::create(crl_path).context("Unable to create CRL")?
        };
